...
*Response:* The turbines would be funded by tax dollars, meaning increased taxes for residence. More left leaning communities are usually more supportive of increased taxes for community benefits and clean energy.
...
*Response:* Spillover matters because it can result in the treatment effect leaning towards 0.
...
*Response:* In this study there are far more untreated than treated precincts. By matching, we can validate causal inference and limit
...
*Response:* You need multiple pre-treatment covariates and many control observations.
...
*Response:* After matching, the love plot shows that the difference in means of the covariates.
...
*Response:* When using a matching model, we run the risk of not including covariates.
...
```{r}
model1_ff <- lm(
    proposed_turbine ~ perc_lib + precinct_id + year,
