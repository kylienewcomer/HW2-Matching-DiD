...
*Response:* The STUVA assumption is likely violated from spillover effect. It is likely that a proposed turbine in a different precinct would still impact other precincts, because the turbine can be seen from greater than 3km away.
...
*Response:* In this study, spillover is regarded as spillover from precincts due to people seeing turbines from further away than the 3 km threshold. Stokes accounts for this by excluding precincts less than 6km from wind turbines as the control, creating a buffer.
...
*Response:* In this study there are far more untreated than treated precincts. By matching, we can validate causal inference and limit any bias through treatment assignment. There is also variation between precincts, so matching will pair control and treated precincts with similar demographics that potentially influence voting.
...
*Response:* The SE increased from 0.031 to **0.039**, most likely due to the fact that the variance of the residuals were not constant. A robust standard error is able to account for heteroskedasticity, which increased SE estimates and makes a more conservative estimate for effect.
...
Your interpretation is missing a comparison — make sure to note that the decrease in liberal vote
share is relative to a counterfactual, not simply a standalone change. Also, units should be
percentage points, not percent

*Response:* The Stokes 2015 study found that proposed wind turbines decreased the vote for the incumbent party by about 0.05 relative to the control, which is similar to our results of a decrease by 0.057. While both estimates show a decrease in incumbent vote shares, our estimate was less precise as seen in the larger p value It is likely different due to the random sampling of 40 precincts used to simulate the data, and a more general specification of the model.
...
*Response (4-6 sentences):* On average, proposed turbines within 3km of a precinct are estimated to decrease the incumbent vote share by 0.1, compared to the control. This is taking into account the fixed effects of unobserved variables that may impact indirectly impact the voting results. Clustering the precincts is important because the precinct are repeated in the data for each year. This means that observations of precinct each year are not independent of each other and must be accounted for.
